class File:
    ...
    def open(self):
        if self.is_opened:
            raise ValueError(f"Ошибка. Файл {self.__str__()} уже открыт")  # does not actually work, but the idea is solid
        else:
            self.is_opened = True

    def close(self):
        if not self.is_opened:
            raise ValueError(f"Ошибка. Файл {str(self)} уже закрыт")
        else:
            self.is_opened = False
    ...
    def read(self):
        if self.is_opened:
            ...
            raise ValueError(f"Ошибка чтения. Файл {str(self)} закрыт")
